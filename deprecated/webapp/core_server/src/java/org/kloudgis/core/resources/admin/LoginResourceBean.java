    ...
    public User loggedUser(@Context HttpServletRequest req) {
        ...
        if (session != null) {
            ...
            if (user != null) {
                ...
                Query query = em.createQuery("from UserDbEntity where user_name = :name").setParameter("name", user);
                List<UserDbEntity> lstU = query.getResultList();
                if (lstU.size() == 1) {
                    UserDbEntity usr = lstU.get(0);
                    User pojo = usr.toPojo();
        ...
        throw new EntityNotFoundException("No session established");
    }
}
